...
#### MySQL冗余数据的三种方案
* 水平拆分的数据库非patition key上的查询就需要扫描多个库，使用数据冗余这种反范式设计来满足分库后不同维度的查询需求
* 服务同步双写：由服务层同步写冗余数据
  - 优点：1）不复杂，服务层由单次写，变两次写；2）数据一致性相对较高
  - 缺点：1）请求的处理时间增加；2）数据仍可能不一致，例如第二步写入T1完成后服务重启，则数据不会写入T2
* 服务异步双写
  - 优点：请求处理时间短
  - 缺点：1）系统的复杂性增加了，多引入了一个组件（消息总线）和一个服务（专用的数据复制服务）；2）数据有一个不一致时间窗口；3）在消息总线丢失消息时，冗余表数据会不一致
* 线下异步双写
  - 优点：1）数据双写与业务完全解耦；2）请求处理时间短
  - 缺点：1）数据有一个不一致时间窗口；2）数据的一致性依赖于线下服务或者任务的可靠性
  
#### 秒杀系统架构优化思路
* 秒杀业务为什么难做：多个人读一个数据，读写冲突，锁非常严重
* 优化方向：1）将请求尽量拦截在系统上游；2）充分利用缓存
* 常见秒杀结构：浏览器-站点-服务-数据
* 各层次优化
  - 浏览器，app层：禁止重复提交、限制提交频率
  - 站点层：对uid进行请求计数和去重，其余请求返回缓存内容
  - 服务层：请求队列（有多少库存，透多少请求去数据库，其余返回已售完）；读请求用缓存
  - 数据层：
